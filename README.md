...
```text
This project uses Wayfinder, a supervised-succession protocol. Find its root (README.md + PROTOCOL.md — at the repo root, or in a wayfinder/ subfolder) and read both in full. Then: (1) open adapters/claude-code/claude-md-block.md and add its pointer block to this project's CLAUDE.md (create CLAUDE.md if absent, never overwrite existing content, adjust paths to where Wayfinder actually lives). The adapter also ships an optional /succession skill — do NOT install it yourself; name it to me and wait for my explicit yes. The yes must be one I gave to WORDS that named the skill — a chat reply, or my approval of a plan that listed it explicitly. A bare file-permission popup during setup is not consent; consent is words you can quote back, or it didn't happen. (2) Tell me in plain words how succession will work here and when you'd propose it; (3) locate your class scratchpad per the protocol — read it if it exists, note if it doesn't. Then stop and give me your read: anything unclear or off in these documents, say so plainly — that's part of the job.
```
...
3. **The `/succession` skill is a separate consent.** Name it, explain it's optional agent config, and wait for an explicit yes — a chat reply, or the human's approval of a plan that names the skill in so many words. A bare file-permission popup is not consent. The receipt rule covers consent too: if you can't quote the words the human said yes to, it didn't happen. Never install it as an unnamed part of "setup."
