...
1. **The packet** — from [`line/templates/packet.md`](line/templates/packet.md), written while the elder is sharp. Contents, in order:
   ...
   - **The continuity check** — questions testing **inhabitation, not recall**, in the **5+2 shape** (field rule, 2026-07-08): five on the situation (this run's rules, wounds, sequences) plus two on POSTURE — how to hold and use the things, not what they are (*"when your list empties, what do you do?" · "what must your own loop prompt carry, and why?"*). Written FRESH by each elder per succession — the shape is the invariant, the content never is.
...
**The held-directly rule (field finding, 2026-07-07).** Whoever grades, audits, or gap-analyzes holds the primary texts directly — packets, answers, protocols, shipped framings are never read *for* you by an agent. Agents may fetch satellite receipts (the quotes out of a five-megabyte transcript); the synthesis is done by the mind holding the primaries. Partial-context delegation is how audits fail politely — the same way handoffs do. (The human's raw words, with the decode marked as a decode: *"the ship framings themselves and must hold these directly, not through an agent."*) Its working form is **scout-then-look**: agents return LOCATIONS (paths, line ranges, whereabouts); the frontier mind then reads those primaries itself. The scout finds the door; the mind that decides walks through it.

**Waiting has a shape (field rules, 2026-07-07/08 — the owner's design, matured same night).** An agent waiting on an artifact or event never waits blind: on every self-scheduled wake it checks the disk first (substrate first, always); if nothing has landed, it sends ONE short **duty report** to its supervising mind — not "any updates?" but *"here's my state — what do you need me to do?"* — idle capacity offers itself, and the report doubles as a liveness heartbeat. Then it re-schedules on the **two-tier ladder**: while active or expecting, walk the warm tier (150→200→270s — inside the prompt-cache's ~5-minute warm window, near-free); when truly idle, leap straight to the long tier (1800–3600s); **never crawl the expensive middle** (300–1500s pays a full context re-read per wake for barely longer waits). **The tiers are sensed, not spoken:** presence reads from the substrate — the human's newest activity across session transcripts (quiet ≥ ~45 min → long watch), a watched session's transcript mtime, a new session appearing in the list. Nobody announces "sleeping"; the loops breathe with the human automatically. And the discipline that holds it all: **the loop prompt is a self-packet** — every wake is a fresh invocation and the prompt is the only thing that re-arrives, so an agent entering a loop writes its bindings INTO the prompt (its no-work rules, its tier rules, what outranks what, whose word ends the window) — and **re-schedules at the end of EVERY turn, including answer-turns** (field finding FM-9, hit twice in one night by two different minds: a turn spent answering displaces the pending wake; schedule-at-turn-end is the only compliance, a surviving wake is luck). A **loop-guard hook** (warn on turn-end without a scheduled wake) is the planned adapter enforcement.
...
**When a successor becomes the elder, it refreshes the golden substrate before writing its packet** — a refresh, not a rewrite; that's what makes the project's self-model gain resolution across agents instead of drifting. Two named organs mature the substrate beyond crystallizations (rulings of 2026-07-08, origin project): a **LORE file** — the methodology written down with its scars attached, contract-cadence (accretes slowly, rephrase-never-drop, lean-ness gated) — because working rules must not pass mouth-to-mouth between generations; and a **PRODUCT-MODEL file** per product — the living model with addresses into the code, checked at every work completion — so a new mind reads the model and jumps to the address instead of re-reading the codebase.
